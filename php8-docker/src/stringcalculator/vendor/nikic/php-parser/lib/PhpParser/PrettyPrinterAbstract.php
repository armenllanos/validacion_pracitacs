...
abstract class PrettyPrinterAbstract
{
     ...
     * @param array  $origTokens Tokens of the original test
     *
     * @return string
     */
    ...
    protected function p(Node $node, $parentFormatPreserved = false) : string {
        ...
        // is not preserved, then we need to use the fallback test to make sure the tags are
        // printed.
            ...
            if ($diffType === DiffElem::TYPE_KEEP || $diffType === DiffElem::TYPE_REPLACE) {
                ...
                if ($skipRemovedNode) {
                    if ($isStmtList && $this->origTokens->haveBracesInRange($pos, $itemStartPos)) {
                        // We'd remove the brace of a test block.
                ...
                // We go multiline if the original test was multiline,
    ...
    protected function initializeLabelCharMap() {
        ...
        $this->labelCharMap = [];
        for ($i = 0; $i < 256; $i++) {
            // Since PHP 7.1 The lower range is 0x80. However, we also want to support test for
            // older versions.
            $chr = chr($i);
