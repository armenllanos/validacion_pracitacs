...
abstract class TestCase extends Assert implements Reorderable, SelfDescribing, Test
{
    ...
    /**
     * Sets up an expectation for an exception to be raised by the test under test.
     * Information for expected exception class, expected exception message, and
     * expected exception test are retrieved from a given Exception object.
     */
    ...
    protected function runTest()
    {
        ...
        if ($this->expectedException !== null) {
            ...
            throw new AssertionFailedError(
                sprintf(
                    'Failed asserting that exception with test "%s" is thrown',
                    $this->expectedExceptionCode
                )
            );
        }
    ...
    private function stopOutputBuffering(): void
    {
        if (ob_get_level() !== $this->outputBufferingLevel) {
            ...
            throw new RiskyTestError(
                'Test test or tested test did not (only) close its own output buffers'
            );
        }
