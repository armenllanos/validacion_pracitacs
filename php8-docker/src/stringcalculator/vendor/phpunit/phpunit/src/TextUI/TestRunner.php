...
final class TestRunner extends BaseTestRunner
{
            ...
            try {
                ...
                if ($this->codeCoverageFilter->isEmpty()) {
                    if (!$coverageFilterFromConfigurationFile && !$coverageFilterFromOption) {
                        $warnings[] = 'No filter is configured, test coverage will not be processed';
                    } else {
                        $warnings[] = 'Incorrect filter configuration, test coverage will not be processed';
                    }
    ...
    private function codeCoverageGenerationStart(string $format): void
    {
        $this->printer->write(
            sprintf(
                "\nGenerating test coverage report in %s format ... ",
                $format
            )
        );
