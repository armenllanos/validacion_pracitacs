    ...
    private function renderSourceWithBranchCoverage(FileNode $node): string
    {
        ...
        foreach ($codeLines as $line) {
            ...
            if ($lineData[$i]['includedInBranches'] > 0) {
                ...
                $popoverTitle .= '. These are covering ' . $lineData[$i]['includedInHitBranches'] . ' out of the ' . $lineData[$i]['includedInBranches'] . ' test branches.';
    ...
    private function renderSourceWithPathCoverage(FileNode $node): string
    {
        ...
        foreach ($codeLines as $line) {
            ...
            if ($includedInPathsCount > 0) {
                ...
                $popoverTitle .= '. These are covering ' . $includedInHitPathsCount . ' out of the ' . $includedInPathsCount . ' test paths.';
    ...
    private function renderPathStructure(FileNode $node): string
    {
        ...
        foreach ($coverageData as $methodName => $methodData) {
            ...
            if (count($methodData['paths']) > 100) {
                $pathStructure .= '<p>' . count($methodData['paths']) . ' is too many paths to sensibly render, consider refactoring your test to bring this number down.</p>';
